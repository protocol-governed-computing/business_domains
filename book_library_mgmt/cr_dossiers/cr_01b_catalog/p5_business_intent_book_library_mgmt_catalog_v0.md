...
|------------|------------------------------------------------------------------------------|--------------------|----------------|
| Book record | MUTABLE_STATE | The library requires one authoritative record per book, its bibliographic information is correctable, and its state moves from registered to retired and back on the same record | S4 bm_entities Book |
...
|-----------|-----------------|----------------|
| Each physical copy belongs to exactly one book | A copy the library owns is a copy of one published thing; a copy recorded against two books would make the collection's description untrue | S1 business_invariants #1 |
| Each book the library holds has exactly one authoritative record | The library needs one place that says what it holds, which is the whole point of a governed catalog | S1 business_invariants #2 |
...
|--------|--------|---------|-----------------------------|----------------|
| Register | Book, with its first physical copy | An authorized staff member registers a book the library has acquired | IN_SCOPE | S4 capability_graph Register a book together with its first physical copy |
