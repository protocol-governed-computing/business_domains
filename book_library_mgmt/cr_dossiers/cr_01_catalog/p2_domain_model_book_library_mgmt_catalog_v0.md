# Domain Model — book_library_mgmt / catalog
...
| Audit append | ai_governance::CC_APPEND_AUDIT_EVENT_V0 | Appends one performed action to a durable audit record. | PARTIAL | It appends another subdomain's audit shape into another subdomain's store. |
...
| Append-only record | capability_side_effects::CS_APPENDONLY_JSONL_V0 | Appends an entry to a durable record that is never rewritten. | EXACT | It records what happened; it decides nothing about what may happen. |
...
| No rule enforces that a copy belongs to exactly one work. | CRITICAL | The invariant the business states most firmly has nothing enforcing it. | OPEN | S1 business_invariants #1 |
