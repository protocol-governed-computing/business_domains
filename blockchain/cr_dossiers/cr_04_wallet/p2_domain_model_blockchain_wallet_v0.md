...
| The act that creates a wallet reads records identity owns and declares nothing about them. | VERIFIED | `blockchain::WF_CREATE_WALLET_V0` reaches `blockchain::CC_RESOLVE_ACTOR_V0`, whose two steps read `CONTACT_ADDRESS_REGISTRY` and then `ACTORS`. Both are declared by `blockchain::STRUCTURE_IDENTITY_STORAGE_V0` and reached only by `blockchain::RB_IDENTITY_BINDINGS_V0`, which the act names nowhere. | S1 system_beliefs #7 |
| The business permits an act to declare the bindings it consults, and refuses a write through one. | VERIFIED | `fb.runtime_binding::CONSTITUTION_RUNTIME_BINDING_V0` §2a states the resolution model and `fb.runtime_binding::INVARIANT_RB_STORAGE_SUBDOMAIN_OWNED_V0` holds a binding to describing only its own subdomain's records. The composition seals an act's storage from the binding it owns and those it consults, marking every record owned or consulted, and refuses a write against a consulted one. | S1 system_beliefs #8 |
| A person nobody accepted is given a wallet today. | VERIFIED | Registering a person and never deciding about them, then dispatching the act, returns SUCCESS and records a wallet against them. The act takes the address off the record it read and never reads the state beside it. | S1 system_beliefs #9 |
...
| The act reads records it declares no reach to. | CRITICAL | The act asks for a record it never said it would read, which is invisible until it runs. | VERIFIED | S1 system_beliefs #7 |
| A person the business has not accepted is given a wallet. | CRITICAL | The business's own rule, stated plainly and enforced nowhere. | VERIFIED | S1 system_beliefs #9 |
