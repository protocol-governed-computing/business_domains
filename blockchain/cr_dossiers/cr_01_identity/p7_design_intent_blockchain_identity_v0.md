...
| Admit a request to register a person | IN | blockchain::IN_REGISTER_ACTOR_V0 | A request to admit a person as an actor | identity | NEW | S5 provisional_codes IN_REGISTER_ACTOR_V0 |
| Admit a request to record a verification decision | IN | blockchain::IN_RECORD_VERIFICATION_DECISION_V0 | A request to record a decision, carrying the authority, the outcome and the grounds | identity | NEW | S5 provisional_codes IN_RECORD_VERIFICATION_DECISION_V0 |
...
|----------|------|----------------------------------------|---------|----------------|
| blockchain::WF_REGISTER_ACTOR_V0 | blockchain::IN_REGISTER_ACTOR_V0 | IN | ACK -> blockchain::CC_VALIDATE_REGISTRATION_V0; NACK -> EXIT_REJECTED | S7 new_artifacts IN_REGISTER_ACTOR_V0 |
...
| blockchain::WF_RECORD_VERIFICATION_DECISION_V0 | blockchain::IN_RECORD_VERIFICATION_DECISION_V0 | IN | ACK -> blockchain::CC_RESOLVE_ACTOR_V0; NACK -> EXIT_REJECTED | S7 new_artifacts IN_RECORD_VERIFICATION_DECISION_V0 |
...
|---------|------|-----------|------------|---------------|-----------|-------|----------|----------|---------|----------------|-----------------|-----------|
| blockchain::CC_VALIDATE_REGISTRATION_V0 | 1 | read_registration | capability_transforms::CT_PURE_VALIDATE_RECORD_STRUCTURE_V0 | CT | VALIDATE_RECORD_STRUCTURE | — | record, rules | validation_status | SUCCESS -> continue; VIOLATION -> exit | — | SUCCESS | in: record=record, rules=rules; out: validation_status=validation_status |
| blockchain::CC_CLAIM_CONTACT_ADDRESS_V0 | 1 | extract_address | capability_transforms::CT_PURE_EXTRACT_V0 | CT | EXTRACT | — | record, field | value | SUCCESS -> continue; VIOLATION -> exit | — | SUCCESS | in: record=record, field=field; out: value=value |
...
| blockchain::CC_RECORD_VERIFICATION_DECISION_V0 | 1 | read_state_admits_decision | capability_transforms::CT_PURE_VALIDATE_SET_MEMBERSHIP_V0 | CT | VALIDATE_SET_MEMBERSHIP | — | value, admitted | membership_status | SUCCESS -> continue; VIOLATION -> exit | — | SUCCESS | in: value=value, admitted=admitted; out: membership_status=membership_status |
| blockchain::CC_RECORD_VERIFICATION_DECISION_V0 | 2 | read_outcome_admitted | capability_transforms::CT_PURE_VALIDATE_SET_MEMBERSHIP_V0 | CT | VALIDATE_SET_MEMBERSHIP | — | value, admitted | membership_status | SUCCESS -> continue; VIOLATION -> exit | — | SUCCESS | in: value=value, admitted=admitted; out: membership_status=membership_status |
| blockchain::CC_RECORD_VERIFICATION_DECISION_V0 | 3 | refuse_self_verification | capability_transforms::CT_PURE_COMPARE_EQUAL_V0 | CT | COMPARE_EQUAL | — | left, right | equal | SUCCESS -> continue; VIOLATION -> exit | — | SUCCESS | in: left=left, right=right; out: equal=equal |
...
| blockchain::CC_VALIDATE_REGISTRATION_V0 | read_registration | INPUT | rules | inputs.registration_rules | S7 cc_composition read_registration |
| blockchain::CC_VALIDATE_REGISTRATION_V0 | read_registration | OUTPUT | validation_status | results.validation_status | S7 cc_composition read_registration |
| blockchain::CC_CLAIM_CONTACT_ADDRESS_V0 | extract_address | INPUT | record | inputs.actor_record | S7 cc_composition extract_address |
| blockchain::CC_CLAIM_CONTACT_ADDRESS_V0 | extract_address | INPUT | field | inputs.address_field | S7 cc_composition extract_address |
| blockchain::CC_CLAIM_CONTACT_ADDRESS_V0 | extract_address | OUTPUT | value | results.contact_address | S7 cc_composition extract_address |
| blockchain::CC_CLAIM_CONTACT_ADDRESS_V0 | claim_address | INPUT | key | results.contact_address | S7 cc_composition claim_address |
| blockchain::CC_CLAIM_CONTACT_ADDRESS_V0 | claim_address | OUTPUT | address | results.claimed_address | S7 cc_composition claim_address |
...
| blockchain::CC_RECORD_VERIFICATION_DECISION_V0 | read_state_admits_decision | INPUT | admitted | inputs.states_admitting_a_decision | S7 cc_composition read_state_admits_decision |
| blockchain::CC_RECORD_VERIFICATION_DECISION_V0 | read_state_admits_decision | OUTPUT | membership_status | results.state_admits_decision | S7 cc_composition read_state_admits_decision |
...
| blockchain::CC_RECORD_VERIFICATION_DECISION_V0 | read_outcome_admitted | INPUT | admitted | inputs.admitted_outcomes | S7 cc_composition read_outcome_admitted |
| blockchain::CC_RECORD_VERIFICATION_DECISION_V0 | read_outcome_admitted | OUTPUT | membership_status | results.outcome_admitted | S7 cc_composition read_outcome_admitted |
...
| blockchain::CC_RECORD_VERIFICATION_DECISION_V0 | refuse_self_verification | OUTPUT | equal | results.authority_is_the_subject | S7 cc_composition refuse_self_verification |
...
|----------|--------------------------------------|-------|------|--------------------|---------|---------|
| blockchain::IN_REGISTER_ACTOR_V0 | INPUT | actor_record | object | YES |  | The details the person supplies about themselves |
| blockchain::IN_REGISTER_ACTOR_V0 | INPUT | registration_rules | array | YES |  | The rules the supplied details are read against for absence and form |
| blockchain::IN_RECORD_VERIFICATION_DECISION_V0 | INPUT | contact_address | string | YES |  | The address naming the actor the decision is about |
| blockchain::IN_RECORD_VERIFICATION_DECISION_V0 | INPUT | verifying_authority | string | YES |  | The authority making the decision, recorded and never resolved |
| blockchain::IN_RECORD_VERIFICATION_DECISION_V0 | INPUT | decision | string | YES |  | The outcome, which must be one of the two admitted values |
| blockchain::IN_RECORD_VERIFICATION_DECISION_V0 | INPUT | grounds | string | NO |  | The reason stated for the decision; required for a rejection |
...
|---------|-----------|-----|-------|----------------|
| blockchain::RB_IDENTITY_BINDINGS_V0 | capability_side_effects::CS_MUTABLE_JSON_V0 | root | blockchain/identity | S6 storage_governance A durable record of every person the business knows, carrying whether it has accepted them |
| blockchain::RB_IDENTITY_BINDINGS_V0 | capability_side_effects::CS_REGISTRY_V0 | root | blockchain/identity | S6 storage_governance An atomic claim on each contact address |
| blockchain::RB_IDENTITY_BINDINGS_V0 | capability_side_effects::CS_APPENDONLY_JSONL_V0 | root | blockchain/identity | S6 storage_governance An unamendable trail of every occurrence recorded against an actor |
